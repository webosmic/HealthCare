...
class AllUsers extends Component {
    ...
    if(this.props.userType === 'patient'){
      if(navigator.geolocation){
        navigator.geolocation.getCurrentPosition(function(position){
          ...
          let query = `https://api.betterdoctor.com
          /2016-03-01/doctors?location=${lat}%2C${lon}%2C100&user_
          location=${lat}%2C${lon}&skip=0&limit=20&user_
          key=bdd1495417e49ba2f1aa40461ce8f17d`;
          ...
          .end((err, result) => {
              ...
              result.body.data.map((doctor,index) => {
                docs.push({
                  ...
                  bd_uid: doctor.uid,
                  practice_city: doctor.practices[0].visit_address.city ?
                  doctor.practices[0].visit_address.city : null,
                  practice_state: doctor.practices[0].visit_address.state ?
                  doctor.practices[0].visit_address.state : null
                });
              })
  ...
  searchSubmit(search) {
    let that = this;
    let qName = this.state.doc_name ?
      `name=${this.state.doc_name}`:'';
    let qQuery = this.state.doc_query ?
      `&query=${this.state.doc_query}`:'';
    let qSpecialty = this.state.doc_specialty ?
      `&specialty_uid=${this.state.doc_specialty}` : '' ;
    let qLocation = (this.state.doc_state && this.state.doc_city) ?
      `&location=${this.state.doc_state}-${this.state.doc_city.trim().replace(' ','-')}`: '';
    let qGender =  this.state.doc_gender ?
      `&gender=${this.state.doc_gender}`: '';
    let qSort = this.state.doc_sort ?
      `&sort=${this.state.doc_sort}`: '';
  ...
  componentWillReceiveProps(nextProps) {
    this.setState({
      doc_query: nextProps.searchVals.docQuery ?
        nextProps.searchVals.docQuery.toLowerCase() : null,
      doc_name: nextProps.searchVals.docName ?
        nextProps.searchVals.docName.toLowerCase() : null,
      doc_specialty: nextProps.searchVals.docSpecialty ?
        nextProps.searchVals.docSpecialty.toLowerCase() : null,
      doc_gender: nextProps.searchVals.docGender ?
        nextProps.searchVals.docGender.toLowerCase() : null,
      doc_sort: nextProps.searchVals.docSort,
      doc_state: nextProps.searchVals.docState ?
        nextProps.searchVals.docState.toLowerCase() : null,
      doc_city: nextProps.searchVals.docCity ?
        nextProps.searchVals.docCity.toLowerCase() : null
    })
      ...
      return (
          ...
          <div className="allPhysicians">
            ...
            {this.state.currentDocs.map((doc,index) => {
              return (
                <li
                  key={index}>
                  <Link
                    to={"/patient/physicians/"+doc.betterDoctorUID+"/individual"}
                  >
                <img
                ...
                />
                <p
                  className="physicianInfo">{doc.first} {doc.last}
                </p>
                <br/>
                <p
                  className="physicianSpecialty">{doc.specialty}
                </p>
                </Link></li>
              )
            })}
            ...
            <form
              ...
              onSubmit={handleSubmit(props => this.searchSubmit(props))}
            >
              <div>
                <h2>
                ...
                <Field
                  placeholder="Search by name, location, practice, specialty"
                  name="docQuery"
                  type="text"
              ...
              <h3> OR </h3>
              <p
                className="helpfulInfo"
              >
                City/State or Provider Name must be provided
              </p>
              <br/>
              <span>
                <label
              ...
              <span>
                ...
                >
                  ...
                  >
                    ...
                    <option
                      value="Ear, Nose and Throat Doctor"
                    >
                    Ear, Nose and Throat Doctor
                    </option>
                    <option
                      value="Emergency Medicine Doctor"
                    >
                    ...
                    <option
                      value="Geriatric Medicine Doctor"
                    >
                    Geriatric Medicine Doctor
                    </option>
                    <option
                      value="Hospice Care and Palliative"
                    >
                    Hospice Care and Palliative
                    </option>
                    <option
                      value="Infectious Disease Doctor"
                    >
                    ...
                    <option
                      value="Neuromusculoskeletal Medicine"
                    >
                    ...
                    <option
                      value="Preventetive Medicine Doctor"
                    >
                    Preventetive Medicine Doctor
                    </option>
                    <option
                      value="Prmary Care Doctor"
                    >
                    Prmary Care Doctor
                    </option>
                    <option
              ...
              <span>
                ...
                >
                  ...
                  <Field
                    ...
                    <option
                      value="best-match-asc"
                    >
                      Best Match Ascending
                    </option>
                    <option
                      value="best-match-desc"
                    >
                      Best Match Descending
                    </option>
                    <option
                      value="distance-asc"
                    >
          ...
          {this.state.docs.map((doc,index) => {
            return (
              ...
              >
              <Link
                to={"/patient/physicians/"+doc.bd_uid+"/individual"}
                >
              <div
              ...
              <p
                className="physicianSpecialty"
              >
                {doc.practice_city}, {doc.practice_state}
              </p>
              </Link>
            </li>
            )
